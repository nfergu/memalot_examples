...
4. The AI tool should be able to instrument this code, analyse the leak report via the Memalot MCP server, and fix the memory leak.

AI tools _may_ find the cause of leak even without Memalot, since this is a simple example. But they may not find it at all. For example, without Memalot some tools decide that the leak is in the `DataHolder.__hash__` method, and ignore the real bug.
